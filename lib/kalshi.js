...
/** Explicit time range, for the backtest's paged history walk. */
export function getCandlesRange(ticker, intervalMin, startTs, endTs) {
  return getJSON(
    `/markets/${encodeURIComponent(ticker)}/candlesticks` +
      `?period_interval=${intervalMin}&start_ts=${startTs}&end_ts=${endTs}`,
    { ttlMs: 60_000 },
  );
}
