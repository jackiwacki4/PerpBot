...
import { applyChosenStrategy, chosenStatus } from './chosen.js';
...
async function handleSnapshot(res, ticker) {
  ...
  sendJSON(res, 200, {
    ...
    // The strategy that survived the search, applied to this market's live
    // bars. Null when no strategy passed, in which case the page says so
    // rather than showing a call nothing supports.
    tested: applyChosenStrategy(snapshot.bars1m, snapshot.funding.history),
    testedStatus: chosenStatus(),
  });
}
