...
/** No perp contract trades anywhere near this; anything above it is corrupt. */
const MAX_SANE_CONTRACT_PRICE = 1e9;
/**
 * Biggest believable move between two consecutive one-minute closes. Real
 * crypto never does this in a minute, and it holds across Kalshi's two-hour
 * maintenance gap too, where prices resume within a fraction of a percent.
 */
const MAX_ONE_MINUTE_FACTOR = 1.5;
...
function toBars(candlesticks = [], contractSize) {
  ...
  for (const c of candlesticks) {
    ...
    // Around Kalshi's daily maintenance window some candles come back with a
    // sentinel instead of a price (values near 2^62), and occasionally with a
    // print at a fraction of the real price. Left alone these wreck every
    // average on the page and make the chart unreadable, so anything that
    // can't be a real one-minute move is rejected rather than smoothed.
    const plausible = (v) => {
      if (!Number.isFinite(v) || v <= 0 || v > MAX_SANE_CONTRACT_PRICE) return false;
      if (!Number.isFinite(prevClose) || prevClose <= 0) return true;
      const ratio = v / prevClose;
      return ratio > 1 / MAX_ONE_MINUTE_FACTOR && ratio < MAX_ONE_MINUTE_FACTOR;
    };

    const pick = (raw) => {
      const v = num(raw, 0);
      if (plausible(v)) return v;
      if (plausible(bookMid)) return bookMid;
      return prevClose;
    };
    ...
    // Carried through so a backtest can charge the spread that actually
    // existed at the time instead of assuming a flat cost.
    const bidOk = plausible(bidClose) ? bidClose : NaN;
    const askOk = plausible(askClose) ? askClose : NaN;
    ...
    bars.push({
      ...
      bid: bidOk / contractSize,
      ask: askOk / contractSize,
