...
function trendFactor(bars) {
  ...
  return {
    key: 'trend',
    label: 'Trend (EMA 9 vs 21)',
    score: squash(spreadInAtr, 0.8),
    value: `${fast > slow ? 'up' : 'down'} · ${spreadInAtr.toFixed(2)} ATR apart`,
    detail:
      fast > slow
...
function bookFactor(book, mid, contractSize) {
  ...
  return {
    key: 'book',
    label: `Order book (±${DEPTH_BAND_PCT}%)`,
    score: squash(imbalance, 0.45),
    value: `${(imbalance * 100).toFixed(0)}% ${imbalance >= 0 ? 'bid' : 'ask'}`,
...
function fundingFactor(funding) {
  ...
  return {
    ...
    value: `${(rate * 100).toFixed(4)}% / ${FUNDING_PERIOD_HOURS}h (${annualPct >= 0 ? '+' : ''}${annualPct.toFixed(1)}% yr)`,
    detail:
      rate > 0
...
export function computeSignals(snapshot, now = Date.now()) {
  ...
  const factors = [
    trendFactor(bars1m),
    momentumFactor(bars1m),
    meanReversionFactor(bars1m),
